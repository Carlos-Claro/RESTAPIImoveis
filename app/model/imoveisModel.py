...
class imoveisModel(object):
    ...
    def getItens_integra(self, data):
        query = {}
        query['colunas'] = 'imoveis.id as _id, imoveis.id as id, imoveis.nome as nome, imoveis.descricao as descricao, imoveis.referencia as referencia, imoveis_tipos.nome as imoveis_tipos_titulo, imoveis_tipos.english as imoveis_tipos_english, imoveis_tipos.link as imoveis_tipos_link, imoveis_tipos.id as imoveis_tipos_id, imoveis.id_tipo as id_tipo, IF ( imoveis.comercial = 1, "Comercial", IF ( imoveis.residencial = 1, "Residencial", "Lazer") ) as uso, imoveis.comercial as comercial, imoveis.residencial as residencial, imoveis.lazer as lazer, imoveis.mostramapa as mostramapa, imoveis.condominio as terreno, imoveis.quartos as quartos, imoveis.garagens as garagens, imoveis.banheiros as banheiros, imoveis.quartos as quartos, imoveis.suites as suites, imoveis.mobiliado as mobiliado, imoveis.cobertura as cobertura, imoveis.condominio as condominio, imoveis.condominio_valor as condominio_valor, imoveis.novo as novo, imoveis.area as area, imoveis.area_terreno as area_terreno, imoveis.area_util as area_util, imoveis.cep as cep, IF ( imoveis.id_logradouro > 0, logradouros.logradouro, imoveis.logradouro ) as logradouro, imoveis.numero as numero, bairros.nome as bairro, bairros.link as bairros_link, IF ( imoveis.bairro <> bairros.nome, imoveis.bairro, "") as vila, imoveis.bairro_combo as bairro_combo, bairros.cidade as bairro_cidade, cidades.link as cidades_link, cidades.link as cidade_link, imoveis.id_cidade as imovel_id_cidade, imoveis.id_cidade as id_cidade, cidades.id as cidades_id, cidades.nome as cidade_nome, cidades.uf as uf, cidades.nome as cidade, estados.nome as estado, logradouros.logradouro as logradouro_, imoveis.longitude as longitude, imoveis.latitude as latitude, IF( imoveis.latitude <> "", CONCAT( imoveis.latitude, ", ", imoveis.longitude ), "" ) as mapa, imoveis.venda as venda, imoveis.locacao as locacao, imoveis.locacao_dia as locacao_dia, imoveis.venda as tipo_venda, imoveis.locacao as tipo_locacao, imoveis.locacao_dia as tipo_locacao_dia, IF ( imoveis.venda = 1, "venda", IF ( imoveis.locacao = 1, "locacao", "locacao_dia" ) ) as tipo, if ( imoveis.venda = 1, "venda", IF ( imoveis.locacao = 1, "locação", IF ( imoveis.locacao_dia = 1, "locação temporada", NULL ) ) ) as imovel_para, if ( imoveis.venda = 1, "venda", IF ( imoveis.locacao = 1, "locacao", IF ( imoveis.locacao_dia = 1, "locacao_dia", NULL ) ) ) as tipo_negocio, IF ( imoveis.preco_venda > 0, imoveis.preco_venda, IF ( imoveis.preco_locacao > 0, imoveis.preco_locacao, imoveis.preco_locacao_dia ) ) as preco, imoveis.preco_venda as preco_venda, imoveis.preco_locacao as preco_locacao, imoveis.preco_locacao_dia as preco_locacao_dia, CONCAT(imoveis.preco_venda, "-", imoveis.preco_locacao, "-", imoveis.preco_locacao_dia) as valores, empresas.id as id_empresa, empresas.empresa_nome_fantasia as nome_empresa, empresas.empresa_nome_fantasia as imobiliaria_nome, empresas.nome_seo as imobiliaria_nome_seo, empresas.empresa_telefone as imobiliaria_telefone, empresas.whatsapp as imobiliaria_whatsapp, empresas.pagina_logo_pequeno as logo, empresas.empresa_numero as imobiliaria_numero, empresas.empresa_email as empresa_email, empresas.empresa_emaillocacao as locacao_email, empresas.pagina_creci as creci, empresas.mudou as mudou, end_empresa.cidade as imobiliaria_cidade, end_empresa.logradouro as imobiliaria_logradouro, end_empresa.bairro as imobiliaria_bairro, empresa_cidade.ddd as ddd, empresas.servicos_sms_limite as sms_limite, empresas.empresa_fone_sms as empresa_telefone_sms, empresas.pagina_limite_ofertas as pagina_limite_ofertas, hotsite_parametros.sms_quem as sms_quem, imoveis.video as video, imoveis.data_atualizacao as data_atualizacao, imoveis.views as views, imoveis.ordem_rad as ordem, imoveis.destaque_bairro as destaque_bairro, imoveis.destaque_tipo as destaque_tipo, IF ( imoveis_corretor.recebe_email = 1, imoveis_corretor.email, "") as email_corretor, imoveis_corretor.nome as nome_corretor, imoveis_corretor.sms as sms_corretor, imoveis_corretor.celular as celular_corretor, CONCAT(imoveis.invisivel, "-", imoveis.reservaimovel, "-", imoveis.vendido, "-", imoveis.locado) as status, imoveis_situacao.titulo as situacao_titulo, imoveis_situacao.link as situacao_link'
        query['tabela'] = 'imoveis'
        query['join'] = [
                {'tabela':'empresas','where':'imoveis.id_empresa = empresas.id','tipo':'INNER'}
                ,{'tabela':'imoveis_tipos','where':'imoveis.id_tipo = imoveis_tipos.id','tipo':'LEFT'}
        ...
        #    query['where'].append('imoveis.id_cidade in (2)')
        ...
        retorno['itens'] = {}
        for item in itens:
            retorno['qtde'] = retorno['qtde']+1
            retorno['itens'][item['id']] = item
            retorno['itens'][item['id']]['images'] = self.getImagesIDimovel(item['id'])
        return retorno
